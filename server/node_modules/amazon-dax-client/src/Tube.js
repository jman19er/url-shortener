...
class SocketTubePool {
  ...
  _alloc(wait) {
    ...
    let localSessionVersion = this._sessionVersion;
    const socket = this._connector.connect(() => this.socketCallback(socket, localSessionVersion)).on('error', (e) => this.socketError(wait, e));
  }

  socketCallback(socket, localSessionVersion) {
    let newTube = new ClientTube(socket, localSessionVersion, this._credProvider, this._region);
